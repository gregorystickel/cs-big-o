...
function findFourth(numbers) {
    ...
    for (let i = 0; i < numbers.length; i++) { // O(1) ? O(n)
        if (count === 4) { // O(1)
...
// 7) -------------------------------------------------------
let numbers = [76,35,73,13,4,88,23,12,86,1,54,9,10,2] // O(1)
// remember numbers could be reassigned, added to, etc.
let newNumbersA = numbers.filter((num) => { //O(n)
  return num < 5
})
...
let newNumbersB = numbers.map((num) => { // O(n)
  return num * 3 
})
...
function arrInception(parentArr) {
    for (let i = 0; i < parentArr.length; i++) {
      ...
      for (let j = 0; j < childArr.length; j++) {
        console.log(`value at index ${j} in child arr (at ${i} in parent arr): ${childArr[j]}`) // O(1)
      }
    }
  }
  // Q: What is the space complexity of this function?
...
function countLetters(str) {
  ...
  const splitStr = str.split('') // O(n)

  splitStr.forEach(char => { // O(n)
    if (letters.hasOwnProperty(char)) { // O(1)
      letters[char]++ // O(n)
    }
  })
...
// ANSWER: Each depends on size of the string returned
